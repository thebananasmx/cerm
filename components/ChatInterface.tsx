...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ onComplete, diseases }) => {
  ...
  useEffect(() => {
    ...
    if (lastMessageRef.current) {
      ...
      // Offset reducido para mobile (160px) vs desktop (200px) debido a la compactación
      const offset = isMobile ? 160 : 200; 
  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-4 sm:py-8 min-h-screen flex flex-col relative">
      {/* Header de Pasos Sticky - Compactado en mobile mb-3 */}
      <div className="sticky top-20 z-40 bg-cerm-light/95 backdrop-blur-md py-3 sm:py-6 mb-3 sm:mb-8 border-b border-cerm-green/10 animate-in fade-in duration-500 shadow-sm sm:shadow-none">
      ...
      <div className="flex-grow flex flex-col">
        {/* Espaciado de mensajes reducido en mobile: space-y-8 y mb-10 */}
        <div className="space-y-8 sm:space-y-24 mb-10 sm:mb-32 flex-grow">
          {messages.map((msg, idx) => {
            ...
            return (
              <div 
                ...
                className={`flex ${isModel ? 'justify-start' : 'justify-end'} animate-in fade-in slide-in-from-bottom-6 duration-700`}
              >
                <div className={`${isModel 
                  ? 'text-xl sm:text-5xl font-bold serif-font text-cerm-dark leading-tight max-w-4xl scroll-mt-32 sm:scroll-mt-48' 
          ...
          {isTyping && (
            <div className="flex items-center gap-3 text-cerm-green text-[8px] sm:text-[10px] font-black uppercase tracking-[0.2em] animate-pulse">
        ...
        {/* Opciones con padding reducido en mobile para ahorrar espacio */}
        {!isTyping && options.length > 0 && (
          <div className="mt-auto grid grid-cols-1 sm:grid-cols-2 gap-3 py-6 sm:py-12 border-t border-cerm-border bg-cerm-light/50">
            {options.map((opt, i) => (
              <button 
                key={i} 
                onClick={() => handleOptionSelect(opt)} 
                className="group flex items-center justify-between p-4 sm:p-7 bg-white border border-cerm-border rounded-[1rem] sm:rounded-[1.5rem] hover:border-cerm-green hover:bg-cerm-surface transition-all text-left shadow-sm hover:shadow-premium active:scale-95"
              >
